...
library SuperTokenV1Library {
    ...
    function increaseFlowRateAllowance(
        ...
        host.callAgreement(
            cfa,
            abi.encodeCall(
                cfa.increaseFlowRateAllowance,
                (token, flowOperator, addedFlowRateAllowance, new bytes(0))
            ),
            userData
        );
        return true;
        ...
        )
    {
        ...
        {
            (
                uint256 lastUpdatedCFA,
                int96 cfaNetFlowRate,
                uint256 cfaDeposit,
                uint256 cfaOwedDeposit
        ...
        {
            (
                uint256 lastUpdatedGDA,
                int96 gdaNetFlowRate,
                uint256 gdaDeposit
            ) = gda.getAccountFlowInfo(token, account);
    ...
    {
        (, IGeneralDistributionAgreementV1 gda) = _getHostAndGDA(token);
        (lastUpdated, flowRate, deposit) = gda.getAccountFlowInfo(
            token,
            account
        );
    ...
    function getBufferAmountByFlowRate(
        ISuperToken token,
        int96 flowRate
    ) internal view returns (uint256 bufferAmount) {
        (, IConstantFlowAgreementV1 cfa) = _getHostAndCFA(token);
        ...
        )
    {
        ...
        (, permissionsBitmask, flowRateAllowance) = cfa.getFlowOperatorData(
            token,
            sender,
            flowOperator
        );
        ...
        returns (int96 actualFlowRate, int96 totalDistributionFlowRate)
    {
        (, IGeneralDistributionAgreementV1 gda) = _getHostAndGDA(token);
        return
            gda.estimateFlowDistributionActualFlowRate(
                token,
                from,
                to,
                requestedFlowRate
    ...
    function estimateDistributionActualAmount(
        ...
        (, IGeneralDistributionAgreementV1 gda) = _getHostAndGDA(token);
        return
            gda.estimateDistributionActualAmount(
                token,
                from,
                to,
                requestedAmount
    ...
    function updateMemberUnits(
        ...
        ) = _getAndCacheHostAndGDA(token);
        host.callAgreement(
            gda,
            abi.encodeCall(
                gda.updateMemberUnits,
                (pool, memberAddress, newUnits, new bytes(0))
            ),
            userData
        );
    ...
    function claimAll(
        ...
        host.callAgreement(
            gda,
            abi.encodeCall(gda.claimAll, (pool, memberAddress, new bytes(0))),
            userData
        );
    ...
    function disconnectPool(
        ...
        host.callAgreement(
            gda,
            abi.encodeCall(gda.disconnectPool, (pool, new bytes(0))),
            userData
        );
        return true;
    }
    ...
    function distribute(
        ...
        host.callAgreement(
            ...
            abi.encodeCall(
                gda.distribute,
                (token, from, pool, requestedAmount, new bytes(0))
            ),
            userData
        );
        return true;
    ...
    function distributeFlow(
        ...
        return
            distributeFlow(token, from, pool, requestedFlowRate, new bytes(0));
    }
    ...
    function distributeFlow(
        ...
        host.callAgreement(
            ...
            abi.encodeCall(
                gda.distributeFlow,
                (token, from, pool, requestedFlowRate, new bytes(0))
            ),
            userData
        );
        return true;
    ...
    bytes32 private constant _GDA_SLOT =
        0xc36f6c05164a669ecb6da53e218d77ae44d51cfc99f91e5a125a18de0949bee4;
    ...
    function _getHostAndCFA(
        ISuperToken token
    ) private view returns (ISuperfluid host, IConstantFlowAgreementV1 cfa) {
        // check if already in contract storage...
        assembly {
            // solium-disable-line
            host := sload(_HOST_SLOT)
