...
contract SuperfluidFrameworkDeployer is SuperfluidFrameworkDeploymentSteps {
        ...
        returns (ISETH nativeAssetSuperToken)
    {
        ...
        string memory superTokenKey = string.concat(
    ...
    {
        PureSuperToken pureSuperTokenProxy = TokenDeployerLibrary
    ...
    function _handleResolverList(
        ...
        if (address(testResolver) == address(0))
            revert RESOLVER_LIST_REQUIRES_DEPLOY_PERIPHERALS();
        if (_listOnResolver) {
    ...
    function _deployWrapperSuperToken(
        ...
        underlyingToken = TokenDeployerLibrary.deployTestToken(
            _underlyingName,
            _underlyingSymbol,
            _decimals,
            _mintLimit
        ...
        string memory underlyingTokenKey = string.concat(
    ...
    modifier requiresSuperTokenFactory() {
        if (address(superTokenFactory) == address(0))
            revert DEPLOY_SUPER_TOKEN_REQUIRES_DEPLOY_SUPER_TOKEN_CONTRACTS();
        _;
    }
